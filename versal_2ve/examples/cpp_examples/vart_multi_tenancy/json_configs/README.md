...
This document explains the structure and usage of the JSON configuration files for the `vart_multi_tenancy` application.

## Overview

The JSON file contains an **array** of model objects. Each object describes one model to be loaded and inferred in its own thread. The application launches one thread per model entry, so the array length determines the degree of parallelism.
...
- `temporal_config.json` — two models sharing the same NPU columns (temporal sharing)
- `spatial_config.json` — two models on separate NPU columns (spatial sharing)
- `temporal_spatial_config.json` — combined temporal + spatial sharing (three models)
