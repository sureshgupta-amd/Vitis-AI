...
static void v_scaler_top(
		...
		U16 StrideOut,
#endif
		U8 InPixelFmt, U8 OutPixelFmt,
#if (FLOAT_OUTPUT == 1)
		U8 FloatOutPixelFmt,bool float_out,
#endif
		U32 PixelRate, U32 LineRate,
...
static void preProcessKernel(HSC_STREAM_MULTIPIX &srcStrm,
							 HSC_STREAM_MULTIPIX &dstStrm,
#if (FLOAT_OUTPUT == 1)
							 HSC_STREAM_FLOAT_MULTIPIX &dstStrmf,
#endif
							 F32 alpha_reg[3], F32 beta_reg[3], int HeightOut,
							 int WidthOut, int ColorModeOut
#if (FLOAT_OUTPUT == 1)
							 ,
							 bool float_out
#endif
)
{
	...
	I16 y, k, z;
	I16 bPassThru = 0;
	int a[HSC_NR_COMPONENTS], b[HSC_NR_COMPONENTS];
	float af[HSC_NR_COMPONENTS], bf[HSC_NR_COMPONENTS];
	...
	for (int j = 0; j < HSC_NR_COMPONENTS; j++)
	{
...
#endif
		{
			...
			a[j] = static_cast<I32>(alpha_reg[j]);
			b[j] = static_cast<I32>((beta_reg[j]) * (1 << 16));
		}
	}
	...
	if (float_out)
	{
		YUV_FLOAT_MULTI_PIXEL out_pix, temp_pix;
		YUV_MULTI_PIXEL in_pix;
		for (y = 0; y < HeightOut; ++y)
		{

			for (z = 0; z < WidthOut / HSC_SAMPLES_PER_CLOCK; ++z)
			{
...
#pragma HLS PIPELINE II = 1
				...
				for (int i = 0; i < HSC_SAMPLES_PER_CLOCK; i++)
				{
					for (int j = 0; j < HSC_NR_COMPONENTS; j++)
					{
						ap_uint<HSC_BITS_PER_COMPONENT> temp =
							in_pix.val[i * HSC_NR_COMPONENTS + j];
						F32 x = static_cast<F32>(temp);
						F32 out;
						...
						out_pix.val[i * HSC_NR_COMPONENTS + j] = out;
						/*bPassThru temp for other formats */
						temp_pix.val[i * HSC_NR_COMPONENTS + j] = x;
					}
				}
				dstStrmf << (bPassThru ? temp_pix : out_pix);
			}
		}
	}
	else
...
#pragma HLS PIPELINE II = 1
				...
				for (int i = 0; i < HSC_SAMPLES_PER_CLOCK; i++)
				{
					for (int j = 0; j < HSC_NR_COMPONENTS; j++)
					{
						...
						ap_uint<HSC_BITS_PER_COMPONENT> x =
							in_pix.val[i * HSC_NR_COMPONENTS + j];
						int out;
...
#define F32_VALUE_FROM_AXIMM_ARRAY(x) reinterpret_cast<F32&>(u32TempArray[x])
...
#if (NORMALIZATION==1)
	Multi_Sc.params_alpha_0 = F32_VALUE_FROM_AXIMM_ARRAY(26);
	Multi_Sc.params_alpha_1 = F32_VALUE_FROM_AXIMM_ARRAY(27);
	Multi_Sc.params_alpha_2 = F32_VALUE_FROM_AXIMM_ARRAY(28);
	Multi_Sc.params_beta_0 = F32_VALUE_FROM_AXIMM_ARRAY(29);
	Multi_Sc.params_beta_1 = F32_VALUE_FROM_AXIMM_ARRAY(30);
	Multi_Sc.params_beta_2 = F32_VALUE_FROM_AXIMM_ARRAY(31);
...
#pragma HLS INTERFACE m_axi port=ms_maxi_srcbuf offset=slave bundle=mm_video latency=100 depth=32000000 num_write_outstanding=NUM_OUTSTANDING num_read_outstanding=NUM_OUTSTANDING max_write_burst_length=BURST_LENGTH max_read_burst_length=BURST_LENGTH
...
#else
#pragma HLS INTERFACE m_axi port=ms_maxi_dstbuf offset=slave bundle=mm_video latency=100 depth=32000000 num_write_outstanding=NUM_OUTSTANDING num_read_outstanding=NUM_OUTSTANDING max_write_burst_length=BURST_LENGTH max_read_burst_length=BURST_LENGTH
...
#pragma HLS ARRAY_PARTITION variable=vfltCoeff complete dim=2
	...
	while (RdnxtDesc != 0)
	{
		...
		if( ( Multi_Sc.msc_outPixelFmt < 0 ) || ( Multi_Sc.msc_outPixelFmt > 52 ))
		{
...
// This avoids adding a mux in the DMA pipeline.
#if (FLOAT_OUTPUT == 1 && MAX_NR_PLANES == 3)
	// For B_G_R_FP32: swap dst plane0 and plane2 pointers so that the R_G_B planar DMA path writes R->buf2, G->buf1, B->buf0
#if (HAS_B_G_R_FP32 == 1)
	if (Multi_Sc.msc_outPixelFmt == B_G_R_FP32) {
		AXIMM tmp = dst0;
		dst0 = dst2;
		dst2 = tmp;
	}
...
#if (FLOAT_OUTPUT == 1)
bool float_out = 0;
U8 tempPixelFmt = Multi_Sc.msc_outPixelFmt;
/*
Perform color conversion and scaling in INT8 formats
Write the data in F32 after preprocessing
*/
if (MEMORY2LIVE[Multi_Sc.msc_outPixelFmt] == float_mode)
{
    float_out = 1;
    switch (Multi_Sc.msc_outPixelFmt)
    {
        case RGB_FP32:
        case RGB_BF16:
        case RGB_FP16:
...
#if (FLOAT_OUTPUT == 1)
				tempPixelFmt,(U8) Multi_Sc.msc_outPixelFmt,  float_out,
#else
...
#if (FLOAT_OUTPUT == 1)
#pragma HLS stream depth=16 variable=dstStrmf
#endif
...
#pragma HLS PIPELINE II=1
	...
	{       
	        ...
	        if (float_out)
		{
			WidthOutBytes = WidthOut * BYTES_PER_PIXEL[FloatOutPixelFmt];
		}
		else
#endif
		{
	...
	preProcessKernel(stream_out, dstStrm,
#if (FLOAT_OUTPUT == 1)
			dstStrmf,
#endif
			alpha_reg, beta_reg, HeightOut, WidthOut, ColorModeOut
#if (FLOAT_OUTPUT == 1)
			, float_out
#endif
			);
...
#if (FLOAT_OUTPUT == 1)
			, FloatOutPixelFmt, float_out
#endif
			);

#else  //NORMALIZATION end
